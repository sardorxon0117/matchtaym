...
import { pickTodayOrNextPerLeague } from "@/lib/match-format";
...
export default async function PromoBoxServer({ banners }: { banners: Banner[] }) {
  ...
  // One match each from Premier League, La Liga and Bundesliga — today's
  // match in that league if there is one, otherwise the next upcoming one.
  const heroFixtures = pickTodayOrNextPerLeague(fixtures, PROMO_LEAGUE_IDS);
